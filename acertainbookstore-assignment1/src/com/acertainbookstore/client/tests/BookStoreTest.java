...
public class BookStoreTest {
	...
	public void testRateBooks() throws BookStoreException {
		...
		bookToRate.add(new BookRating(TEST_ISBN, 5));
		...
		List<StockBook> listBooks = storeManager.getBooksByISBN(isbnList);
		assertTrue(listBooks.size() == 1);
		StockBook bookInList = listBooks.get(0);

		assertTrue(bookInList.getISBN() == TEST_ISBN && bookInList.getAverageRating() == 5
				&& bookInList.getNumTimesRated() == 1 && bookInList.getTotalRating() == 5);
	}
	...
	public void testRateInvalidISBN() throws BookStoreException {
		...
		booksToRate.add(new BookRating(-1, 5));
		...
		// to check the books' pre and post state are the same
		assertTrue(bookInList.getISBN() == addedBook.getISBN() && bookInList.getTitle().equals(addedBook.getTitle())
				&& bookInList.getAuthor().equals(addedBook.getAuthor()) && bookInList.getPrice() == addedBook.getPrice()
				&& bookInList.getNumSaleMisses() == addedBook.getNumSaleMisses()
				&& bookInList.getAverageRating() == addedBook.getAverageRating()
	...
	/**
	 * Tests that books with non-existing ISBNs cannot be rated.
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	...
	public void testRateNonExistingISBN() throws BookStoreException {
		HashSet<BookRating> booksToRate = new HashSet<BookRating>();
		booksToRate.add(new BookRating(TEST_ISBN, 5)); // valid
		// Try to rate a book with non-existing ISBN.
		booksToRate.add(new BookRating(100000, 5));
	...
	/**
	 * Tests that books with invalid score cannot be rated.
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	@Test
	public void testRateInvalidScore() throws BookStoreException {
		HashSet<BookRating> booksToRate = new HashSet<BookRating>();
		// Try to rate a book with invalid score large than 5
		booksToRate.add(new BookRating(TEST_ISBN, 7));
	...
	/**
	 * Tests that books with negative score cannot be rated.
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	@Test
	public void testRateNegativeScore() throws BookStoreException {
		HashSet<BookRating> booksToRate = new HashSet<BookRating>();
		// Try to rate a book with negative score
	...
	public void testGetTopRatedBooks() throws BookStoreException {
		...
		HashSet<BookRating> booksToRate = new HashSet<BookRating>();
		booksToRate.add(new BookRating(TEST_ISBN, 2));
		booksToRate.add(new BookRating(TEST_ISBN + 1, 5));  // top rated book
		client.rateBooks(booksToRate);

		List<Book> topRatedBooks = client.getTopRatedBooks(1);
		assertTrue(topRatedBooks.size() == 1);
		Book top1Book = topRatedBooks.get(0);
		assertTrue(top1Book.getISBN() == TEST_ISBN + 1 && top1Book.getAuthor().equals("Donald Knuth")
				&& top1Book.getPrice() == (float) 300 && top1Book.getTitle().equals("The Art of Computer Programming"));
	}

	/**
	 * Tests that you can't get negative number of top books.
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	...
	public void testGetNegativeTopRatedBooks() throws BookStoreException {
		...
		try {
			// try to get negative top rated books
